...
import { fieldTypes, Form } from '../../../../appcraft-forms/src'
...
const defaultFields = [
  {type: "grid", columns: [
    { fixedWidth: 140, fields: [
      {name: "avatar", label: "Avatar", type: "file", width: 130, height: 110},
    ]},
    { fields: [
      {name: "firstname", label: "First Name", type: "string", width: '33%'},
      {name: "lastname", label: "Last Name", type: "string", width: '33%'},  
      {name: "label", label: "Label", type: "computed", width: '33%', template: "{{firstname}} {{lastname}}"},  
...
export class DataEditor extends React.Component {
  ...
  constructor(props){
    ...
    this.onChange = (formName, fieldName, value) => {
      ...
      const data = {
        ...(onChange ? this.props.data : this.state.data),
        [fieldName]: value
      }
      if (onChange){
        onChange(data)
  ...
  render(){
    const { fields=defaultFields, size="medium", data, errors, onChange } = this.props
    return (
      <div className="c-text">
        <Container size={size} style={{padding: 8}}>
          <Form fieldTypes={fieldTypes} 
                fields={fields}
                errors={errors}
                className="acf-material"
